...
class WebSecurityScanner:
    ...
    def check_csrf(self, url: str) -> None:
        ...
        try:
            ...
            for form in forms:
                has_csrf_token = any(
                    token in form.attrs.get('action', '') or form.prettify().lower()
                    for token in ['csrf', 'xsrf', 'token']
                )
                if not has_csrf_token:
                    self.report_vulnerability({
                        'type': 'CSRF Protection Missing',
                        'url': url,
                        'form_action': form.attrs.get('action', 'Unknown')
                    })
    ...
    def check_clickjacking(self, url: str) -> None:
        ...
        try:
            ...
            if 'deny' not in x_frame_options and 'sameorigin' not in x_frame_options \
                    and 'frame-ancestors' not in content_security_policy:
                self.report_vulnerability({
                    'type': 'Clickjacking',
                    'url': url,
            ...
            print(f"Error testing Clickjacking on {url}: {str(e)}")
